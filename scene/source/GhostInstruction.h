...
// ghost runs around and scares people

class GhostInstruction
	: public UnigineInstruction
{
public:
	virtual ~GhostInstruction() = default;
	GhostInstruction(uint8_t sequence_position, NodePtr destination = nullptr, float arrive_radius = 0.1f, bool interruptable = true)
	{
	...
	virtual void init() override
	{
		for (auto& scared_person : m_scared_persons)
		{
			...
			GOB::GOAL flee;
			if (person)
				flee = person->m_reasoner.get_goal("fliehen");
			if (flee)
				flee->set_value(1.0f);
		}
	}

	void add_scared_person(NodePtr person)
	{
...
protected:
	...
	std::vector<NodePtr> m_scared_persons;
